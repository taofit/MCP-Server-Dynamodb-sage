...
export default function ActivityPage() {
  ...
  }, [notifications, range, search]);

  const toggle = (key: string) =>
    setExpanded((prev) => ({ ...prev, [key]: !prev[key] }));

  const successful = notifications.filter((n) => n.severity === "success").length;
  const failed = notifications.filter((n) => n.severity === "error").length;
